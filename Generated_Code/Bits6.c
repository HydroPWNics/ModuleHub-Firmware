...
} Bits6_TDeviceData;                   /* Device data structure type */

typedef Bits6_TDeviceData *Bits6_TDeviceDataPtr ; /* Pointer to the device data structure. */

static const uint32_t Bits6_PIN_MASK_MAP[2U] = {
   0x01000000U, 0x02000000U
};                                     /* Map of masks for each pin */

/* {Default RTOS Adapter} Static object used for simulation of dynamic driver memory allocation */
static Bits6_TDeviceData DeviceDataPrv__DEFAULT_RTOS_ALLOC;
...
LDD_TDeviceData* Bits6_Init(LDD_TUserData *UserDataPtr)
{
  ...
  Bits6_TDeviceDataPtr DeviceDataPrv;
  ...
  GPIOE_PDDR |= GPIO_PDDR_PDD(0x03000000);
  /* Set initialization value */
  /* GPIOE_PDOR: PDO&=~0x03000000 */
  GPIOE_PDOR &= (uint32_t)~(uint32_t)(GPIO_PDOR_PDO(0x03000000));
  ...
  PE_LDD_RegisterDeviceStructure(PE_LDD_COMPONENT_Bits6_ID,DeviceDataPrv);
...
void Bits6_SetDir(LDD_TDeviceData *DeviceDataPtr, bool Dir)
{
  ...
  if (Dir) {
    /* Output */
    GPIO_PDD_SetPortOutputDirectionMask(Bits6_MODULE_BASE_ADDRESS, Bits6_PORT_MASK);
  } else {
    /* Input */
    GPIO_PDD_SetPortInputDirectionMask(Bits6_MODULE_BASE_ADDRESS, Bits6_PORT_MASK);
  }
}
...
uint32_t Bits6_GetVal(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  (void)DeviceDataPtr;                 /* Parameter is not used, suppress unused argument warning */
  if ((GPIO_PDD_GetPortDirection(Bits6_MODULE_BASE_ADDRESS) & Bits6_PORT_MASK) != 0U) {
    /* Port is configured as output */
    PortData = GPIO_PDD_GetPortDataOutput(Bits6_MODULE_BASE_ADDRESS) & Bits6_PORT_MASK;
  } else {
    /* Port is configured as input */
    PortData = GPIO_PDD_GetPortDataInput(Bits6_MODULE_BASE_ADDRESS) & Bits6_PORT_MASK;
  }
  return PortData >> Bits6_PIN_ALLOC_0_INDEX; /* Return port data shifted with the offset of the first allocated pin*/
}
...
void Bits6_PutVal(LDD_TDeviceData *DeviceDataPtr, uint32_t Val)
{
  ...
  RawVal = (Val & Bits6_PORT_VALID_VALUE_MASK) << Bits6_PIN_ALLOC_0_INDEX; /* Mask and shift output value */
  /* Set port data by toggling the different bits only */
  GPIO_PDD_TogglePortDataOutputMask(Bits6_MODULE_BASE_ADDRESS,
      (GPIO_PDD_GetPortDataOutput(Bits6_MODULE_BASE_ADDRESS) ^ RawVal) & Bits6_PORT_MASK);
}
...
LDD_TError Bits6_GetBit(LDD_TDeviceData *DeviceDataPtr, uint8_t Bit, bool *BitVal)
{
  ...
  Mask = Bits6_PIN_MASK_MAP[Bit];
  if ((GPIO_PDD_GetPortDirection(Bits6_MODULE_BASE_ADDRESS) & Mask) != 0U) {
    /* Port is configured as output */
    PortVal = GPIO_PDD_GetPortDataOutput(Bits6_MODULE_BASE_ADDRESS);
  } else {
    /* Port is configured as input */
    PortVal = GPIO_PDD_GetPortDataInput(Bits6_MODULE_BASE_ADDRESS);
  }
  if ((PortVal & Mask) != 0U) {
    *BitVal = (bool)TRUE;
  } else {
...
LDD_TError Bits6_PutBit(LDD_TDeviceData *DeviceDataPtr, uint8_t Bit, bool Val)
{
  ...
  Mask = Bits6_PIN_MASK_MAP[Bit];
  if (Val) {
    GPIO_PDD_SetPortDataOutputMask(Bits6_MODULE_BASE_ADDRESS, Mask);
  } else { /* !Val */
    GPIO_PDD_ClearPortDataOutputMask(Bits6_MODULE_BASE_ADDRESS, Mask);
  } /* !Val */
  return ERR_OK;
}
